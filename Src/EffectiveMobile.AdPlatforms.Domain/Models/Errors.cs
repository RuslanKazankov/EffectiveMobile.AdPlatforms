...
public static class Errors
{
    public static Error InvalidFileSplitSymbol(int lineNumber) =>
        new Error("001", $"Invalid file format: only 1 symbol ':' must be on line {lineNumber}");

    public static Error InvalidFileFormatLocation(int lineNumber) =>
        new Error("002", $"Invalid file format: location must start with '/' and location name must be larger then 1 on {lineNumber}");

    public static Error MissingLocationParameter { get; } =
        new Error("003", "Location parameter is required");
    
    public static Error MissingPlatformParameter { get; } =
        new Error("004", "Platform parameter is required");
